...
function connectTheme(themeName, styles, WrappedComponent) {
  if (!styles) {
    return class ThemeComponent extends React.Component {
      render() {
        ...
        let themeStyle;
        if (themeName) {
          const bundleName = WrappedComponent.bundleName || props.bundleName || findParentInstanceBundleName(this);
          ...
          // 支持包内的common
          const bcStyles = (bundleName && currentTheme[`${bundleName}.common`]) || {};
          themeStyle = {
            ...extStyles,
            ...bcStyles,
            ...theStyles
  ...
  return themes.connectStyle(themeName, styles)(WrappedComponent);
}

export function connectStyle(themeName, styles = null) {
  if (typeof themeName === 'string') {
    return (WrappedComponent) => connectTheme(themeName, styles, WrappedComponent);
  } else {
